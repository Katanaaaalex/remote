...
`git checkout master` - вернуться к актуальному ee состоянию, затем продолжить работееу

`git diff`- показать разницу межgbvду текущим файлом и закоммиченным файлом.

`git tag` - используется для захвата некой точки в истории, которая используется для релиза нумерованной версии.
